...
def journalist_agent(state: AgentState) -> AgentState:
    ...
    2. Then analyze the news and create a 8 point plain text summary in genZ slang where each point must be self explanatory with reader understandability
        ...
        response = agent.invoke(
            {
                "messages": [
                    HumanMessage(
                        content="""Summarize the most latest news from internet into 8 point wise plain text summary in genZ slang where each point must be self explanatory with reader understandability. 
